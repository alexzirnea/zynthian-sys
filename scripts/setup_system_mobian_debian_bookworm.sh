...
[ -n "$ZYNTHIAN_SYS_REPO" ] || ZYNTHIAN_SYS_REPO="https://github.com/alexzirnea/zynthian-sys.git"
...
[ -n "$ZYNTHIAN_SYS_BRANCH" ] || ZYNTHIAN_SYS_BRANCH="mobian_debian12_port"
...
apt-get -y install systemd avahi-daemon dhcpcd-dbus usbutils
#PORT #usbmount exfat-utils

apt-get -y install xinit xserver-xorg-video-fbdev x11-xserver-utils xinput libgl1-mesa-dri
apt-get -y install xfwm4 xfce4-panel xdotool cpufrequtils
#PORT #vnc4server
#PORT #xfwm4-themes
...
# CLI Tools
apt-get -y install raspi-config psmisc tree joe nano vim p7zip-full i2c-tools ddcutil
#PORT #raspi-config
...
#Tools
apt-get -y --no-install-recommends install build-essential git swig subversion pkg-config autoconf automake premake gettext intltool libtool libtool-bin cmake cmake-curses-gui flex bison ngrep qt5-qmake qt4-qmake gobjc++ ruby rake xsltproc vorbis-tools zenity
#PORT #qt5-default-found only in buster, needs searching on bookworm

# AV Libraries => WARNING It should be changed on every new debian version!!
apt-get -y --no-install-recommends install libavcodec59 libavformat59 libavutil57 libavresample4 libavformat-dev libavcodec-dev
...
#PORT #laditools liblash-compat-dev libqt4-dev
...
# Python
#PORT Do we ever migrate to python3? #apt-get -y install python python-dev cython python-dbus python-setuptools
apt-get -y install python3 python3-dev cython3 python3-cffi python3-tk python3-dbus python3-mpmath python3-pil python3-pil.imagetk python3-setuptools python3-numpy python3-evdev 2to3
#PORT python3-pyqt4
if [ "$ZYNTHIAN_INCLUDE_PIP" == "yes" ]; then
    apt-get -y install python3-pip
	#PORT remove python2 pip  #python-pip
fi
